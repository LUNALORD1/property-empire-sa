...
import { useLedger, usePlayerProperties, useProfile } from "@/lib/data-hooks";
import { formatZAR } from "@/lib/format";
import { netWorth } from "@/lib/game";
import { TrendingUp, TrendingDown, Wallet, Building2, CreditCard, Sigma } from "lucide-react";
...
export const Route = createFileRoute("/_app/finances")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Cash, debt, net worth, and your monthly income vs expenses." },
    ],
  }),
...
function FinancesPage() {
  ...
  const monthlyExpense = (properties ?? []).reduce((s, p) => s + Number(p.monthly_maintenance), 0);
    ...
    <div className="p-4 max-w-3xl mx-auto w-full overflow-y-auto space-y-5 pb-8">
      ...
      <div className="rounded-2xl p-5 bg-gradient-card border border-primary/20 shadow-gold relative overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-3 mt-5 relative">
          ...
          <Stat icon={<CreditCard className="w-3.5 h-3.5" />} label="Debt" value={formatZAR(debt)} />
        </div>
      </div>
